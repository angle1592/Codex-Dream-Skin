...
This Windows-focused downstream is based on upstream v1.5.11. It adds one launch/management shortcut, per-theme task-page strength, and automatic recovery when a saved theme is selected after the watcher stopped.

Its v1.5.11 compatibility baseline includes the Codex 26.727 main-surface and Settings selectors. If the status says running but task pages remain unskinned, verify that the v1.5.11 managed runtime was installed instead of only restoring an older shortcut.
