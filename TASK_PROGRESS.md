...
## Downstream v1.5.12 compatibility merge — verified locally (2026-08-08)

- [merged] Upstream v1.5.12 Windows theme schema, path-containment, package
  timestamp, image-metadata, and shared runtime fixes are retained.
...
## Downstream v1.5.11 compatibility merge — verified locally (2026-08-02)

- [diagnosed] The installed v1.5.6 watcher and CDP endpoint were alive, but Codex
  26.727 changed the main surface to `_MainContentSurface_*`; the old
  `main.main-surface` contract therefore left task routes at fallback scope.
- [merged] Upstream v1.5.11 selectors, renderer readiness, Settings detection,
  verify-script theme loading, safe theme replacement, and Windows bootstrap
  hardening are retained.
- [preserved] The downstream one-click Chinese launcher, tray restore, per-theme
  `art.taskBackgroundStrength`, 200 ms live preview, and stopped-session recovery
  remain present. Persistent installer entries use the System32 PowerShell path.
- [verified] Runtime asset sync, renderer/selector tests, Windows installer static
  checks, managed-runtime replacement, startup readiness, verified-skin
  preservation, payload integrity, Safe CSS, and task-strength tests pass.
- [environment note] The full Windows suite reaches the upstream theme ZIP
  fail-fast recovery case, where the child waits on the shared ThemeImport mutex
  while the live installed tray is present; the isolated suite exceeded five
  minutes. No product assertion failed before the timeout.
